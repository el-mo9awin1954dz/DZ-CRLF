...
const CRLF = args[4];
const RESPONSE = args[5];
const BUGALERT = args[6];
const token = args[7];
const channel = args[8];
const messageme = args[9];
...
if(CRLF === undefined || INJECTION === undefined || RESPONSE === undefined || BUGALERT === undefined || token === undefined || channel === undefined || messageme === undefined) {
...
async function check(i){
    try{
        const response = await Axios({
            method: "GET",
            url: Self.data[i]+CRLF,
            headers: {INJECTION}
        })

        if(JSON.stringify(response.body).indexOf(RESPONSE) !== -1){
            console.log(`${Self.data[i]} VULNERABLE TO HEADER INJECTION (CRLF INJECTION).`)
            Self.results.push(Self.data[i])
            const notifier = require('node-notifier');

            var MSG = messageme;
    // String
            var alert = " BUG FOUND ON :" + Self.data[i]+CRLF + " \n" + MSG;
    // Object
            notifier.notify({
            	title: 'BUG FIND',
                message: alert,
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠿⣧⣤⡤⢽⣧⣤⠤⢿⢤⣴⣿⣤⣤⣤⣿⣤⣤⣼⡦⠤⠴⡶⣿⡇⠀⠀⠀⠀⠀⠀⠀
		...
		console.log('\n\n\x1b[31m',"FOUND BUG || PAYLOAD >> %s || HEADER >> %s",'\x1b[0m',CRLF,INJECTION);
                console.log("\n\nFOUND BUG >>> ",Self.data[i]+CLRF+" |HEADER| "+INJECTION+"|PAYLOAD|"+CRLF)
		...
		function sendMessage (token, channel, message) {
            ...
            console.log(`${Self.data[i]} NOT VULNERABLE TO HEADER INJECTION (CRLF INJECTION).`)
        }
    }catch{
        console.log(`${Self.data[i]} NOT VULNERABLE TO HEADER INJECTION (CRLF INJECTION).`)
    }
    ...
    if(Self.check_index === Self.data.length){
        ...
        if(!Self.results.length){
            console.log("NO LINKS IS VULNERABLE TO HEADER INJECTION (CRLF INJECTION).")
        }else{
...
//Main
if(!Self_Args.length){
    console.log("node DZ-CRLF.js TARGET-URLS.txt OUTPUT.txt CRLF-PAYLOAD INJECTION-HEADER RESPONSE-CHECK BUG-ALERT TOKEN ID-CHANNEL NOTIFICATION-MSG")
    process.exit()
}
